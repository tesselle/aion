test_that("Fixed from Gregorian - Date", {
  ...
  AD <- as_fixed(1950, 01, 01, calendar = AD())
  BP <- as_fixed(0, 01, 01, calendar = BP())
  ...
  ## Leap year
  R <- as_fixed(year = 2000, month = 02, day = 29, calendar = CE())
...
test_that("Fixed from Gregorian - Fractional year", {
  ...
  fix <- as_fixed(dec, calendar = CE())
  expect_equal(fix, as_fixed(2023, 05, 09, calendar = CE()))
