...
In base R, dates are represented as the number of days since 1970-01-01 (by default), with negative values for earlier dates (see `help(Date)`). **chronos** uses a different approach: it allows to create date vectors represented as *rata die* [@reingold2018], i.e. as number of days since 01-01-01 (Gregorian)^[It is intended that the *rata die* should be an integer value, but this is not enforced in the internal representation.].
